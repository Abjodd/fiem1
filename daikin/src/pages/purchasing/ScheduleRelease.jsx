...
import CreateASN from './createAsn2.jsx'
...
// ═══════════════════════════════════════════════════════════════
// CONFIRM VIEW — uses scheduleReleaseApi.getConfirmData
// Auto-fetches on mount with default date range
// ═══════════════════════════════════════════════════════════════
function ConfirmView({ agreement, onBack }) {
  ...
  const filteredRows = useMemo(() => {
    ...
    return rows.filter(r => terms.some(t =>
  ...
  if (submitted) return (
    <div className="flex flex-col items-center justify-center h-64 gap-4 anim-fade">
      ...
      <button onClick={onBack} className="px-5 h-10 text-[14px] font-semibold text-[#0a6ed1] bg-[#ebf5ff] rounded-lg hover:bg-[#d9ecff] transition-all">
        ← Back to Agreement
      </button>
    </div>
  )
  ...
  return (
    <main className="bg-white anim-fade" style={{ minHeight: 'calc(100vh - 220px)' }}>
      ...
      {/* Table — uses mapped field names from scheduleReleaseApi.getConfirmData */}
          ...
          <table className="w-full text-[13px]" style={{ minWidth: '760px', borderCollapse: 'collapse' }}>
            <thead>
              <tr className="bg-gradient-to-b from-[#fafbfc] to-[#f5f6f7] border-b border-[#e5e5e5] text-[#6a6d70]">
                ...
                {['Item', 'Schedule Line', 'Material', 'Bin', 'Delivery Schedule', 'Confirmed Qty', 'ASN Qty', 'Delivery Date', 'Dispatch Date'].map(h => (
...
export default function ScheduleRelease() {
  ...
  useEffect(() => {
    ...
    scheduleReleaseApi.getScheduleLines(agreement.id, item.itemNo, item.materialNumber, item.deliveryUnit)
  ...
  const drilledItem = selectedItem && agreement ? agreement.items.find(i => i.itemNo === selectedItem.itemNo) : null
  ...
  const filteredItems = useMemo(() => {
    if (!agreement) return []
    return agreement.items.filter(() => true)
  }, [agreement, fromDate, toDate, dateError])

  const handleSelectAgreement = (id) => {
    setSelectedAgreementId(id); setSelectedItem(null)
    setShowConfirmView(false); setMobileSidebarOpen(false)
  }
    ...
    <PageLayout>
      ...
      <div className="bg-[#f5f6f7] min-h-[calc(100vh-104px)]">
        ...
        {showConfirmView && agreement && (
          <ConfirmView agreement={agreement} onBack={() => setShowConfirmView(false)} />
        )}
                      ...
                      <table className="w-full text-[14px]" style={{ minWidth: '480px', borderCollapse: 'collapse' }}>
                        ...
                        <tbody className="row-stagger">
                          {scheduleLines.length === 0 ? (
                            <tr><td colSpan={4} className="py-12 text-center text-[13px] text-[#6a6d70]">No schedule lines</td></tr>
                          ) : scheduleLines.map((line, idx) => (
                            <tr key={`${line.schLineNo}-${idx}`} className="border-b border-[#f0f0f0] last:border-b-0 hover:bg-[#fafbfc] transition-colors">
                              <td className="py-4 px-4">
                                <span className="inline-flex items-center justify-center w-8 h-8 bg-[#ebf5ff] text-[#0a6ed1] rounded-full text-[13px] font-bold">{line.schLineNo}</span>
                              </td>
                              <td className="py-4 px-4 text-[#32363a] font-medium">
                                <div className="flex items-center gap-2">
                                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="text-[#6a6d70] flex-shrink-0"><rect x="3" y="4" width="18" height="18" rx="2"/><path d="M16 2v4M8 2v4M3 10h18"/></svg>
                                  {line.deliveryDate}
                                </div>
                              </td>
                              <td className="py-4 px-4">
              ...
              {agreement ? (
                <>
                          ...
                          {filteredItems.length === 0 ? (
                            ...
                            <tr key={item.itemNo}
                              ...
                              <td className="py-3.5 px-4 text-[#32363a]">
                                {item.deliveredQty} <span className="text-[#6a6d70] text-[12px]">{item.deliveredUnit}</span>
                              </td>
                  ...
                  <div className="px-4 sm:px-6 lg:px-10 py-4 border-t border-[#e5e5e5] flex items-center justify-between gap-3 flex-shrink-0 bg-white shadow-[0_-2px_8px_rgba(0,0,0,0.04)]">
                    {/* Create ASN — disabled-look if any item needs confirmation */}
                    <div className="relative group">
                      ...
                      {asnDisabled(agreement.items) && (
                        <div className="absolute bottom-full left-0 mb-2 px-3 py-1.5 bg-[#32363a] text-white text-[12px] rounded-lg whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-10 shadow-lg">
                          Confirm pending items before creating ASN
