...
// Must be defined here (not inside card functions) so React never
// treats them as new component types on re-render — preserves input focus.
// ═══════════════════════════════════════════════════════════════
...
function MobileItemCard({ item, isSelected, onToggle, onUpdate, onSplitBatch, packagingTypes, pdirOptions }) {
  ...
  // FIX: disable card when avlAsnQty = 0 — these lines have no deliverable qty
  ...
  return (
    <div className={`rounded-xl border mb-3 overflow-hidden transition-colors ${
      isZeroQty
        ? 'bg-[#fafbfc] border-[#e5e5e5] opacity-60'
        : isSelected
          ? 'bg-[#ebf5ff] border-[#0a6ed1]'
      ...
      <div className="flex items-center gap-3 px-4 py-3">
        <input
          type="checkbox"
          checked={isSelected}
          onChange={isZeroQty ? undefined : onToggle}
          disabled={isZeroQty}
          className="accent-[#0a6ed1] w-4 h-4 flex-shrink-0 disabled:cursor-not-allowed disabled:opacity-40"
        />
        ...
        <div className="flex flex-col items-end gap-1 flex-shrink-0">
          {/* itemNo now shows e.g. "10-2" — display only the schedule line part for clarity */}
          <span className="px-2 py-0.5 bg-[#ebf5ff] text-[#0a6ed1] rounded-full text-[11px] font-bold">
            Item {item.ebelp} / Sch {item.schLine}
          </span>
          <span className="px-2 py-0.5 bg-[#f0f4f8] text-[#32363a] rounded text-[11px] font-semibold">{item.storageLocation}</span>
          {isZeroQty && (
            <span className="px-2 py-0.5 bg-[#fce8e6] text-[#cc1c14] rounded text-[11px] font-semibold">No qty</span>
          )}
        </div>
        <button
      ...
      {expanded && (
        <div className="border-t border-[#f0f0f0] bg-[#fafbfc] px-4 py-4">
          <button
            ...
            disabled={isZeroQty}
            className={`w-full flex items-center justify-between gap-2 mb-4 px-3 h-10 text-[13px] font-semibold rounded-lg border transition-all ${
              isZeroQty
                ? 'opacity-40 cursor-not-allowed text-[#6a6d70] bg-[#f5f5f5] border-[#d9d9d9]'
                : 'text-[#0a6ed1] bg-[#ebf5ff] border-[#0a6ed1] hover:bg-[#d9ecff] hover:scale-[1.01] active:scale-[0.99]'
            }`}
          >
          ...
          <div className="mb-3">
            ...
            <input
              ...
              disabled={isZeroQty}
              className="w-full h-10 rounded-lg border border-[#d9d9d9] bg-white px-3 text-[14px] outline-none focus:border-[#0a6ed1] focus:ring-2 focus:ring-[#0a6ed1]/20 transition-all disabled:bg-[#f5f5f5] disabled:cursor-not-allowed"
            />
          </div>
          <div className="mb-3">
...
// ═══════════════════════════════════════════════════════════════
// DESKTOP ITEM CARD
// Field and ReadonlyVal are defined at module scope (above) so React
// never sees them as "new" component types on re-render — preserves
...
function DesktopItemCard({ item, isSelected, onToggle, onUpdate, onSplitBatch, packagingTypes, pdirOptions }) {
  // FIX: disable when avlAsnQty = 0 (DelQty was 0 from SAP — no deliverable qty)
  const isZeroQty = parseFloat(item.avlAsnQty || 0) === 0
  ...
  return (
    <div className={`rounded-xl border transition-all mb-3 overflow-hidden ${
      isZeroQty
        ? 'border-[#e5e5e5] bg-[#fafbfc] opacity-60'
        : isSelected
          ? 'border-[#0a6ed1] bg-[#f5f9ff]'
          : 'border-[#e5e5e5] bg-white hover:border-[#c5d8f0]'
    }`}>

      {/* ── CARD HEADER ── */}
      <div className={`flex items-center gap-3 px-4 py-3 border-b ${
        isZeroQty
          ? 'border-[#f0f0f0] bg-[#fafbfc]'
          : isSelected
            ? 'border-[#cce0f5] bg-[#ebf5ff]'
            : 'border-[#f0f0f0] bg-[#fafbfc]'
      }`}>
        <input
          type="checkbox"
          ...
          onChange={isZeroQty ? undefined : onToggle}
          disabled={isZeroQty}
          className="accent-[#0a6ed1] w-4 h-4 flex-shrink-0 cursor-pointer disabled:cursor-not-allowed disabled:opacity-40"
          title={isZeroQty ? 'No deliverable qty (DelQty = 0) — cannot include in ASN' : undefined}
        />
        {/* Show PO Item and Schedule Line clearly — itemNo is now "10-2" internally */}
        <span className="flex-shrink-0 px-2.5 py-1 bg-[#0a6ed1] text-white rounded-md text-[11px] font-bold tracking-wide">
          Item {item.ebelp}
        </span>
        <span className="flex-shrink-0 px-2 py-1 bg-[#e8f5ec] text-[#107e3e] rounded text-[11px] font-bold">
          Sch {item.schLine}
        </span>
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          {isZeroQty && (
            <span className="px-2 py-0.5 bg-[#fce8e6] text-[#cc1c14] rounded text-[11px] font-semibold">
              No qty available
            </span>
          )}
          <span className="px-2 py-0.5 bg-[#f0f4f8] text-[#32363a] rounded text-[11px] font-semibold">{item.storageLocation}</span>
        ...
        <div className="grid grid-cols-7 gap-3">
          ...
          <Field label="Tax Mismatch">
            <div className="flex items-center gap-2 h-8">
              <button
                onClick={() => !isZeroQty && onUpdate('taxMismatch', !item.taxMismatch)}
                disabled={isZeroQty}
                className={`relative w-10 h-5 rounded-full transition-colors flex-shrink-0 ${item.taxMismatch ? 'bg-[#107e3e]' : 'bg-[#d9d9d9]'} disabled:cursor-not-allowed disabled:opacity-50`}
              >
          ...
          <Field label="Type of Packaging">
            <select
              ...
              onChange={e => onUpdate('packagingType', e.target.value)}
              disabled={isZeroQty}
              className={selectCls + ' disabled:bg-[#f5f5f5] disabled:cursor-not-allowed'}
            >
              <option value="">Select</option>
              {packagingTypes.map(opt => (<option key={opt} value={opt}>{opt}</option>))}
            </select>
          ...
          <Field label="Packing Material Qty" className="max-w-[120px]">
            <input
              ...
              placeholder="0"
              disabled={isZeroQty}
              className={inputCls + ' text-center disabled:bg-[#f5f5f5] disabled:cursor-not-allowed'}
            />
          </Field>
          <Field label="PDIR No.">
            <select
          ...
          {(() => {
            ...
            return (
              <>
                ...
                {!isZeroQty && batchDisabled && !isZeroQty && (
                  <span className="text-[11px] text-[#9ca3af]">Enter Packing Material Qty to enable split</span>
                )}
                {isZeroQty && (
                  <span className="text-[11px] text-[#9ca3af]">No deliverable qty — row excluded from ASN</span>
                )}
                {!batchDisabled && bc > 0 && (
...
export default function CreateASN2({ agreement: propAgreement }) {
  ...
  const selectableItems = useMemo(
    () => items.filter(i => parseFloat(i.avlAsnQty || 0) > 0),
    [items]
  )
  ...
  const toggleOne = (itemNo) => {
    // Guard: never allow selecting zero-qty items
  ...
  // FIX: updateItem uses item.itemNo which is now "10-1", "10-2" etc.
  // Each schedule line is a distinct entry in items[], so only the matching
  // row gets updated — no more cascade across all rows with the same Ebelp.
  ...
  const handleGo = async () => {
    ...
    try {
      ...
      setItems(asnItems)
      // Clear selections that no longer exist in new result set
  ...
  const validateBeforeCreate = () => {
    ...
    selectedItems.forEach(it => {
      // Guard: zero-qty rows should never be selected, but double-check
      const avl = parseFloat(it.avlAsnQty || 0)
      if (avl === 0) {
        errors.push(
          `Item ${it.ebelp} / Sch ${it.schLine} (${it.materialNumber}): ` +
          `Available ASN Qty is 0 — this schedule line has no deliverable quantity.`
        )
        return // skip further checks for this item
      }

      // FG Stock required and must be > avlAsnQty
      const fgVal = parseFloat(it.fgStock)
      if (it.fgStock === '' || isNaN(fgVal)) {
        errors.push(`Item ${it.ebelp} / Sch ${it.schLine} (${it.materialNumber}): FG Stock is required.`)
      } else if (fgVal <= avl) {
        errors.push(
          `Item ${it.ebelp} / Sch ${it.schLine} (${it.materialNumber}): ` +
          `FG Stock (${fgVal}) must be greater than Avl. ASN Qty (${avl}).`
        )
      }
      ...
      if (it.batches && it.batches.length > 0) {
        ...
        if (Math.abs(sum - avl) > 0.0001) {
          errors.push(
            `Item ${it.ebelp} / Sch ${it.schLine} (${it.materialNumber}): ` +
            `batch quantity sum (${sum}) does not match Avl. ASN Qty (${avl}).`
          )
        }
        const bad = it.batches.find(b => !b.batchCode || !b.quantity)
        if (bad) errors.push(
          `Item ${it.ebelp} / Sch ${it.schLine}: incomplete batch row — each row needs Batch/Heat Code and Quantity.`
        )
      }
    })
              ...
              {!itemsLoading && isMobile ? (
                <div className="pt-2">
                  <div className="flex items-center justify-between mb-3">
                    <span className="text-[14px] font-bold text-[#32363a]">
                      Items ({filteredItems.length})
                      {filteredItems.length !== items.length && <span className="text-[#6a6d70] font-normal"> · {items.length - filteredItems.filter(i => parseFloat(i.avlAsnQty||0) === 0).length} with qty</span>}
                    </span>
                  ...
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-[13px] text-[#6a6d70]">
                        ...
                        {filteredItems.filter(i => parseFloat(i.avlAsnQty||0) === 0).length > 0 && (
                          <span className="text-[#9ca3af]"> · {filteredItems.filter(i => parseFloat(i.avlAsnQty||0) === 0).length} with no qty</span>
                        )}
                      </span>
                    </div>
                    {selectedItemNos.length > 0 && (
