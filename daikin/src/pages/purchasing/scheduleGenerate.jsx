...
// ScheduleGridPopup removed — grid opens as /purchasing/schedule-lines page.
...
export default function ScheduleGenerate() {
  ...
  // ── Navigate to ScheduleLines page ──
  const openScheduleLines = (lines, editable, title, mode, pendingIndicator) => {
    navigate('/purchasing/schedule-lines', {
      state: {
        ...
        title,
        mode,
        pendingIndicator,  // 'W' | 'D' | undefined — ScheduleLines passes back in savedLines meta
  ...
  const handleWeek = () => {
    ...
    openScheduleLines(buildLines('week', 0), false, 'Schedule Lines — Week', 'WEEKLY', 'W')
  }
  ...
  const handleDayGenerate = dayCount => {
    ...
    openScheduleLines(buildLines('day', dayCount), false, `Schedule Lines — Day (${dayCount})`, 'DAILY', 'D')
  }
  ...
  const handleEdit = () => {
    ...
    openScheduleLines(lines, true, 'Edit Schedule Lines', '', undefined)
  }
  ...
  if (!agreement) return (
    <PageLayout>
      <div className="flex items-center justify-center h-64 text-[#6a6d70]">No agreements found.</div>
    </PageLayout>
  )
          ...
          <div className="px-4 sm:px-6 lg:px-10 py-3 border-b border-[#e5e5e5] bg-[#fafbfc]">
            <div className="flex flex-col sm:flex-row sm:items-center gap-3">
              ...
              <div className="flex items-center gap-2 flex-shrink-0 flex-wrap">
                ...
                <button onClick={handleApprove} disabled={selArr.length === 0 || saving}
