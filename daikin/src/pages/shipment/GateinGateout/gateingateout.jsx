...
const TABS = [
  {
    ...
    label: 'Timeline',
    icon: (active) => (
      <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke={active ? 'white' : '#0a6ed1'} strokeWidth="2">
  ...
  },
  {
    ...
    label: 'ASN',
    icon: (active) => (
      <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke={active ? 'white' : '#107e3e'} strokeWidth="2">
  ...
  },
  {
    ...
    label: 'Summary',
    icon: (active) => (
      <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke={active ? 'white' : '#cc1c14'} strokeWidth="2">
...
const TIMELINE_STEPS = [
  {
    ...
    icon: (
      <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
        <path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/>
        <path d="M14 2v6h6"/><path d="M12 18v-6M9 15h6"/>
      </svg>
    ),
  },
  {
...
// ═══════════════════════════════════════════════════════════════
// BUTTON VISIBILITY — driven by timeline dates, not status text
// ═══════════════════════════════════════════════════════════════
...
function TrackingPopup({ onSubmit, onCancel, userLoading }) {
  ...
  const handleSubmit = async () => {
    ...
    if (parts.length !== 2 || !parts[0].trim() || !parts[1].trim()) {
      setError('Please enter in format: TrackingNumber/Year  (e.g. 1000001097/2025)')
      return
    }
    ...
    try {
      ...
      const clean = msg.includes('No record found') || msg.includes('404')
        ? 'Tracking number not found. Please check the number and year.'
        : msg.length > 100 ? 'Invalid tracking number or year.' : msg
      setError(clean)
      setLoading(false)
    }
  }
      ...
      >
        ...
        <div className="px-6 py-6">
          ...
          <input
            ...
            type="text"
            value={combined}
            placeholder="e.g. 1000001097/2025"
...
function AsnItemsPopup({ asnNum, fisYear, onClose }) {
  ...
  return (
    ...
    >
      <div
        className="bg-white rounded-2xl shadow-2xl w-full max-w-[560px] overflow-hidden"
      ...
      >
        <div className="px-5 py-4 border-b border-[#e5e5e5] flex items-center justify-between">
          ...
          <button
            onClick={onClose}
            className="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-[#f5f6f7] text-[#6a6d70]"
          >
        ...
        <div className="overflow-x-auto max-h-[400px]">
          {loading ? (
            <div className="py-12 text-center text-[13px] text-[#6a6d70]">
              <div className="w-5 h-5 border-2 border-[#e5e5e5] border-t-[#0a6ed1] rounded-full animate-spin mx-auto mb-2" />
              Loading…
            </div>
          ) : items.length === 0 ? (
            <div className="py-12 text-center text-[13px] text-[#6a6d70]">No items found</div>
          ) : (
            <table className="w-full text-[13px]">
              <thead>
                <tr className="bg-[#f5f6f7] border-b border-[#e5e5e5] text-[#6a6d70]">
                  {['Material', 'Description', 'Quantity', 'Price', 'HSN Code'].map((h) => (
              ...
              <tbody>
                {items.map((it, i) => (
                  <tr key={i} className="border-b border-[#f0f0f0] hover:bg-[#fafbfc]">
                    ...
                    <td className="py-3 px-4 text-[#32363a]">
                      {it.quantity.toFixed(3)} <span className="text-[#6a6d70]">{it.unit}</span>
                    </td>
                    <td className="py-3 px-4 font-semibold text-[#32363a]">
                      {it.price.toFixed(2)} <span className="text-[#6a6d70] text-[11px]">{it.currency}</span>
                    </td>
    ...
    >
      ...
      >
        <div className="flex items-start gap-3 mb-3">
          <div className="w-10 h-10 rounded-full bg-[#e6f4ea] flex items-center justify-center flex-shrink-0 mt-0.5">
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#107e3e" strokeWidth="2">
              <path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><path d="M22 4L12 14.01l-3-3"/>
            </svg>
          </div>
          <div>
            <div className="text-[15px] font-bold text-[#107e3e]">Success</div>
            <div className="text-[13px] text-[#32363a] mt-1 leading-relaxed">{message}</div>
          </div>
        </div>
        <div className="flex justify-end mt-4">
          <button
            onClick={onClose}
            className="px-5 h-9 text-[13px] font-semibold text-white bg-[#0a6ed1] rounded-lg hover:bg-[#085caf] transition-all"
          >
            OK
          </button>
        </div>
...
function ErrorDialog({ message, onClose }) {
  return (
    ...
    >
      ...
      >
        <div className="flex items-start gap-3 mb-3">
          <div className="w-10 h-10 rounded-full bg-[#fce8e6] flex items-center justify-center flex-shrink-0 mt-0.5">
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#cc1c14" strokeWidth="2">
              <circle cx="12" cy="12" r="10"/><path d="M12 8v4M12 16h.01"/>
            </svg>
          </div>
          <div>
            <div className="text-[15px] font-bold text-[#cc1c14]">Error</div>
            <div className="text-[13px] text-[#32363a] mt-1 leading-relaxed">{message}</div>
          </div>
        </div>
        <div className="flex justify-end mt-4">
          <button
            onClick={onClose}
            className="px-5 h-9 text-[13px] font-semibold text-white bg-[#cc1c14] rounded-lg hover:bg-[#a61710] transition-all"
          >
            Close
          </button>
        </div>
      ...
      <div className="anim-fade px-4 sm:px-6 lg:px-10 py-8">
        ...
        <div className="hidden sm:block">
          <div className="relative flex items-start justify-between">
            ...
            {lastIdx > 0 && (
              <div
                className="absolute top-[26px] left-[40px] h-[2px] bg-[#107e3e] z-0 transition-all duration-700"
                style={{ width: `calc(${(lastIdx / (steps.length - 1)) * 100}% - 0px)` }}
              />
            )}
            {steps.map((step, idx) => (
              <div
              ...
              >
                <div
                  className={`w-[52px] h-[52px] rounded-full flex items-center justify-center border-2 transition-all
                    ${step.completed
                      ? 'bg-[#107e3e] border-[#107e3e] text-white shadow-md'
                      : 'bg-white border-[#d9d9d9] text-[#d9d9d9]'}
                    ${idx === lastIdx ? 'scale-110 ring-4 ring-[#107e3e]/15' : ''}`}
                >
                  {step.icon}
                </div>
                <div className="mt-2.5 text-center px-1">
                  <div className={`text-[11px] font-semibold ${step.completed ? 'text-[#32363a]' : 'text-[#b0b4b8]'}`}>
                    {step.label}
                  </div>
                  {step.timestamp && (
                    <div className="text-[10px] text-[#6a6d70] mt-0.5">{step.timestamp}</div>
                  )}
                </div>
              </div>
            ))}
        ...
        <div className="sm:hidden relative pl-10">
          ...
          {lastIdx >= 0 && (
            <div
              className="absolute left-[19px] top-2 w-[2px] bg-[#107e3e] transition-all duration-700"
              style={{ height: `calc(${((lastIdx + 0.5) / steps.length) * 100}%)` }}
            />
          )}
          <div className="space-y-5">
            {steps.map((step) => (
              <div key={step.key} className="relative flex items-start gap-3">
                <div
                  className={`absolute -left-10 w-10 h-10 rounded-full flex items-center justify-center border-2 z-10
                    ${step.completed
                      ? 'bg-[#107e3e] border-[#107e3e] text-white'
                      : 'bg-white border-[#d9d9d9] text-[#d9d9d9]'}`}
                >
                  {step.icon}
                </div>
                <div>
                  <div className={`text-[13px] font-semibold ${step.completed ? 'text-[#32363a]' : 'text-[#b0b4b8]'}`}>
                    {step.label}
                  </div>
                  {step.timestamp && (
                    <div className="text-[11px] text-[#6a6d70]">{step.timestamp}</div>
                  )}
                </div>
              </div>
            ))}
      ...
      <div className="anim-fade px-4 sm:px-6 lg:px-10 py-6">
        <div className="overflow-x-auto rounded-xl border border-[#e5e5e5] shadow-sm">
          <table className="w-full text-[13px]" style={{ minWidth: '700px' }}>
            ...
            <tbody>
              {tracking.asns.length === 0 ? (
                <tr><td colSpan={7} className="py-10 text-center text-[13px] text-[#6a6d70]">No ASN records</td></tr>
              ) : tracking.asns.map((asn, idx) => (
                <tr key={idx} className="border-b border-[#f0f0f0] hover:bg-[#fafbfc] transition-colors">
                  <td className="py-3.5 px-4">
                    <button
                      onClick={() => setAsnPopup({ asnNum: asn.asnNum, fisYear: asn.asnYear })}
                  ...
                  <td className="py-3.5 px-4">
                    <span className="font-semibold text-[#32363a] bg-[#f0f4ff] px-2 py-0.5 rounded text-[11px]">
                      {asn.plant}
                    </span>
                  </td>
          ...
          <table className="w-full text-[13px]" style={{ minWidth: '500px' }}>
            ...
            <tbody>
              {items.length === 0 ? (
                ...
                <tr key={i} className="border-b border-[#f0f0f0] hover:bg-[#fafbfc]">
                  <td className="py-3.5 px-4">
                    <div className="font-bold text-[#32363a]">{it.materialDesc}</div>
                    <div className="text-[11px] text-[#6a6d70] mt-0.5">{it.material}</div>
                  </td>
                  <td className="py-3.5 px-4 text-[#32363a] font-medium">
                    {it.quantity.toFixed(3)} <span className="text-[#6a6d70]">{it.unit}</span>
                  </td>
            ...
            <div className="px-4 sm:px-6 lg:px-10 pt-5 pb-5 border-b border-[#e5e5e5]">
              <div className="flex items-start justify-between flex-wrap gap-4 mb-4">
                ...
                <div className="flex items-center gap-3">
                  <span
                    ...
                    style={{
                      color:
                        t.statusColor === 'green'  ? '#107e3e' :
                        t.statusColor === 'orange' ? '#e76500' :
                        t.statusColor === 'blue'   ? '#0a6ed1' : '#cc1c14',
                    }}
                  >
                    {t.status}
                  </span>
                  <button
                    onClick={handleReset}
                    className="flex items-center gap-1.5 px-3 h-8 text-[12px] font-semibold text-[#6a6d70] border border-[#d9d9d9] rounded-lg hover:border-[#0a6ed1] hover:text-[#0a6ed1] transition-all"
                  >
              ...
              <div className="flex flex-wrap gap-x-16 gap-y-1 text-[13px]">
                <div className="flex-1 min-w-[280px] space-y-1">
                  ...
                  <div className="text-[#6a6d70]">Driver Name: <span className="font-semibold text-[#32363a]">{t.driverName || '—'}</span></div>
                ...
                {TABS.map((tab) => {
                  ...
                  return (
                    <button
                      ...
                      className={`flex flex-col items-center pb-3 border-b-2 transition-all flex-shrink-0
                        ${isActive ? 'border-[#0a6ed1]' : 'border-transparent hover:border-[#d9d9d9]'}`}
                    >
                      <div
                        className={`w-11 h-11 rounded-full flex items-center justify-center mb-1.5 shadow-sm transition-all
                          ${isActive ? 'shadow-md scale-110' : ''}`}
                        style={{ backgroundColor: isActive ? tab.color : '#f0f4f8' }}
                      >
                        {tab.icon(isActive)}
                      </div>
                      <span className={`text-[13px] font-semibold whitespace-nowrap ${isActive ? 'text-[#0a6ed1]' : 'text-[#6a6d70]'}`}>
                        {tab.label}
                      </span>
                    </button>
                  )
        ...
        <div className="fixed bottom-0 left-0 right-0 bg-white/95 backdrop-blur-md border-t border-[#e5e5e5] px-4 sm:px-6 py-3 flex items-center gap-2 z-30 shadow-[0_-4px_16px_rgba(0,0,0,0.06)]">
          ...
          {/* {showGateReport && (
            <button
              ...
              className="flex items-center gap-2 px-4 h-9 text-[13px] font-semibold text-white bg-[#e76500] rounded-lg hover:bg-[#b45309] transition-all shadow-sm disabled:opacity-60"
            >
              {gateRepLoading
                ? <div className="w-4 h-4 border-2 border-white/40 border-t-white rounded-full animate-spin" />
                : (
          ...
          {showGateIn && (
            <button
              ...
              className="flex items-center gap-2 px-4 h-9 text-[13px] font-semibold text-white bg-[#107e3e] rounded-lg hover:bg-[#0b5e2e] transition-all shadow-sm disabled:opacity-60"
            >
              {gateInLoading
                ? <div className="w-4 h-4 border-2 border-white/40 border-t-white rounded-full animate-spin" />
                : (
