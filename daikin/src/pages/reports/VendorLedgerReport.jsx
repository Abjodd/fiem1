...
import { VendorLedgerApi, toSapDate } from '../../services/VendorLedgerReport.js'
...
const MOCK_REPORT_ROWS = [
  ...
  {
    supplier: 'FS859', supplierName: 'Supreme Industries', companyCode: 'DSAL',
    invoiceNumber: '897999', supplierLocation: 'Alwar',
...
// ═══════════════════════════════════════════════════════════════
// MOCK API wrapper — delegates to real service when USE_MOCK=false
// ═══════════════════════════════════════════════════════════════
const api = {
...
// ═══════════════════════════════════════════════════════════════
const COLUMNS = [
  { key: 'supplier',             label: 'Supplier',               w: 130 },
  ...
  { key: 'barcode',              label: 'Barcode',                w: 120 },
  { key: 'basic',                label: 'Basic',                  w: 105 },
  { key: 'taxDetails',           label: 'Tax Details',            w: 110 },
  { key: 'totalInvoiceAmount',   label: 'Total Invoice Amt',      w: 140 },
  { key: 'tds',                  label: 'TDS',                    w: 85  },
  { key: 'others',               label: 'Others',                 w: 85  },
  { key: 'amountPayable',        label: 'Amt Payable',            w: 120 },
  ...
  { key: 'documentType',         label: 'Doc Type',               w: 90  },
  { key: 'documentDate',         label: 'Document Date',          w: 110 },
  { key: 'localCurrencyAmount',  label: 'Local Currency Amt',     w: 145 },
...
// ═══════════════════════════════════════════════════════════════
// DROPDOWN MODAL (searchable, supports multi-select for docType)
// ═══════════════════════════════════════════════════════════════
...
export default function VendorLedgerReport() {
  ...
  const [selectedDocTypes, setSelectedDocTypes] = useState([]) // multi
  const [invoiceNo,      setInvoiceNo]      = useState('')
  const [documentDate,   setDocumentDate]   = useState('')

  // ── Exclusive checkboxes ──
  const [activeCheck, setActiveCheck] = useState('') // '' | 'open' | 'cleared' | 'all'
  ...
  const [ddModal, setDdModal] = useState(null) // null | { field, title, options, multi, selected }
  ...
  // ── Load child suppliers when supplier changes ──
  useEffect(() => {
  ...
  const renderCell = (row, col) => {
    switch (col.key) {
      ...
      case 'clearingDocument':
        return row.clearingDocument ? <span className="text-[#107e3e] text-[12px] font-medium">{row.clearingDocument}</span> : <span className="text-[#d9d9d9]">—</span>
      default:
          ...
          {filterBarVisible && (
            <div className="px-4 sm:px-6 lg:px-10 pt-4 pb-4 flex-shrink-0 border-b border-[#e5e5e5] bg-[#fafbfc] anim-fade">
              ...
              <div className="flex flex-col xl:flex-row gap-4 xl:gap-5">
                ...
                {/* RIGHT: exclusive checkboxes column */}
                <div className="flex flex-row xl:flex-col gap-3 xl:gap-2 xl:pt-4 xl:pl-4 xl:border-l xl:border-[#e5e5e5] xl:min-w-[155px] flex-shrink-0">
                  ...
                  {/* Deselect hint when one is active */}
                  {activeCheck && (
